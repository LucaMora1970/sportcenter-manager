...
// riga — sostituisce l'elenco piatto "Opzione C" di prenota-campo.js. Il
// prezzo NON si mostra più qui: si rivela solo aprendo il pannello di
// conferma di uno slot (vedi apriPannelloPrenota), che ora lo calcola e
// lo mostra in cima. ----------
...
// Un solo pannello di conferma condiviso sotto la griglia (non più uno per
// riga, non ha senso con celle strette) — riapre sempre nello stesso
// punto, evidenzia il bottone scelto.
function selezionaBottoneGriglia(btn) {
...
function renderGrigliaCampi(el, gruppo) {
  ...
  let html = `<div class="grid-wrap"><table class="slot-grid"><tr><th></th>${campi.map(c => `<th><span class="th-pill">${escapeHtml(c.numero)}</span></th>`).join("")}</tr>`;
  fissi.forEach((slot, row) => {
    const durataMinuti = orarioToMin(slot.fine) - orarioToMin(slot.inizio);
  ...
  el.querySelectorAll(".apri-prenota-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      selezionaBottoneGriglia(btn);
      const slot = fissi[parseInt(btn.dataset.row, 10)];
      const campo = campi[parseInt(btn.dataset.col, 10)];
...
// campo condiviso. Le due liste di orari liberi si calcolano
// separatamente (algoritmo anti-buco diverso per ciascuna durata):
// l'unione ordinata degli orari dà le righe, ogni colonna riempie solo le
// proprie. state.durataPadel resta la variabile che confermaPrenota legge
// per durationMinutes — la si imposta qui al click, non più da un toggle
// separato.
function renderGrigliaPadel(el, gruppo) {
  const ora = oraLocaleZurigo();
  const oggi = state.data === ora.dataIso;
  ...
  // Qui l'ora di fine ha senso mostrarla (a differenza di tennis/squash,
  // dove è sempre la stessa durata per tutti): le due colonne durano
  // diverso, l'ora di fine è l'unico modo per vederlo senza doverlo dedurre.
  // Il prezzo si può mostrare subito (a differenza del tennis): per il
  // padel non dipende da chi altro gioca, quindi qui è già quello vero.
  ...
  el.querySelectorAll(".apri-prenota-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const durata = parseInt(btn.dataset.dur, 10);
      state.durataPadel = durata;
      const inizio = minutiToOrario(min);
      apriPannelloPrenota({ campo, inizio, fine: minutiToOrario(min + durata) });
    });
  });
}
...
function render() {
  ...
  // Niente più toggle durata per il padel: le due colonne della griglia
  // mostrano 90' e 60' insieme, vedi renderGrigliaPadel.
  subToggleEl.innerHTML = gruppo.disciplina === "tennis" ? renderPosizioneTennis() : "";
  wirePosizioneTennis();
...
// idx fisso: un solo pannello condiviso alla volta (vedi renderGriglia*),
// non serve più distinguere quale riga l'ha aperto.
function apriPannelloPrenota(slot) {
  const idx = "grid";
  const panel = document.getElementById("grid-panel-area");
  if (!panel) return;

  const nCampi = numeroCampiGiocatore(slot.campo.disciplina);
  giocatoriRisolti = new Array(nCampi).fill(null);

  // Prezzo rivelato solo qui, non più nella griglia (vedi render()) — per
  // il tennis resta indicativo: il prezzo vero dipende anche dalla
  // categoria del secondo giocatore, non ancora nota a questo punto.
  const gruppo = gruppoAttivo();
  ...
  panel.innerHTML = `
    <div class="prenota-panel">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px;">
        <div>
          <div style="font-family:'Space Mono', monospace;font-size:0.74rem;color:var(--chalk-grey);">${escapeHtml(disciplinaLabel(slot.campo.disciplina))} · Campo ${escapeHtml(slot.campo.numero)}</div>
          <span class="pp-orario">${slot.inizio}–${slot.fine}</span>
          <span class="pp-prezzo">${prezzoTitolo}: ${prezzoLabel}</span>
        </div>
